...
void render_hud_stars(void) {
    ...
    if (gCurrLevelNum == LEVEL_CASTLE || gMarioState->numStars > 0) {
       ...
       print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X), HUD_TOP_Y, "$");
        print_text_fmt_int((GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 16), HUD_TOP_Y, "%d", gNumAreaCoinsCollected);
        print_text((14) + GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X - 16),
                        HUD_TOP_Y, "/");
        print_text_fmt_int((4) + GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X - 40),
                        HUD_TOP_Y, "%d", gNumCoinsInArea); 
    }
}
